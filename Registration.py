...
class Registration:
    def __init__(self):
        ...
        img.image = render
        img.place(x=-5, y=-5)
        label1=Label(img,text='Registration Form',font='Arial  25  bold', bg='black', fg='white',width=60)
        ...
        def connect(event):
            ...
            if not nameec.isalpha() or nameec in "'-" :
                ...
                cur = mydb.cursor()
                sql = "insert into studreg(id,name,fname, dob,qualif, gender,email,number,address,streetadrss,city,state,pin) values (%s,%s,%s,%s,%s,%s,%s,%s, %s, %s, %s, %s, %s)"
                a=ide.get()
                b=namee.get()
                c=fnamee.get()
                d=dobe.get()
