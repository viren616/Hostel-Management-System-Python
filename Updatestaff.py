...
class Updatestaff:
    def __init__(self):
        ...
        login = Label(frame1, text='Update Staff Information', font='Algerian  25  ', bg='black', fg='firebrick')
        login.place(x=320, y=0)
        l = Label(canvas, text='Please Enter Details', font='Algerian  25  ', bg='black', fg='firebrick')
        l.place(x=10, y=5)

        username = Label(canvas, text='Staff ID', font='Algerian  15  ', bg='black', fg='firebrick')
        ...
        def proceeed():
            ...
            if (usere.get() == '' or passwe.get() == ''):
                ...
                cur = mydb.cursor()
                cur.execute("select*from staffpass")
                rec = cur.fetchall()
                for x in rec:
                    if (usere.get() == x[0]):
                        if (passwe.get() == x[1]):
        ...
        def connect(event):
            ...
            if not nameec.isalpha() or nameec in "'-" :
                ...
                mydb = mysql.connector.connect(host='localhost', user='root', password='viren', database='python')
                cur = mydb.cursor()
                sql = "update staffreg set name=%s,fname=%s, dob=%s,qualif=%s, gender=%s,email=%s,number=%s,adress=%s,streetadrss=%s,city=%s,state=%s,pin=%s where id=%s "
                a=ide.get()
                b=namee.get()
                c=fnamee.get()
                d=dobe.get()
                ...
                l=state.get()
                m=pin.get()
                value=(b,c,d,e,f,g,h,i,j,k,l,m,a)
                print(value)
                try:
